...
void affichage_profil(BITMAP* buffer,t_joueur tab[], int nbJoueurs,int j)
{
    ...
    BITMAP* paralyse=load_bitmap("Statut/immobile.bmp",NULL);
    BITMAP* protege=load_bitmap("Statut/bouclier.bmp",NULL);
    BITMAP* apeure=load_bitmap("Statut/apeure.bmp",NULL);
    BITMAP* empoisonne=load_bitmap("Statut/poison.bmp",NULL);
